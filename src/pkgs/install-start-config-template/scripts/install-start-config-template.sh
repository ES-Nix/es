...
&& nix flake update --override-input home-manager github:nix-community/home-manager/b372d7f8d5518aaba8a4058a453957460481afbc \
&& git status \
&& git add .
...
AUX=$(nix build --impure --print-out-paths $FLAKE_ATTR)/activate
nix profile remove '.*'
"$AUX"
