      ...
      (final: prev: {
        ...
        prepareVagrantVms = prev.writeScriptBin "prepare-vagrant-vms" ''
          ...
          for i in {0..100};do
            ...
            vagrant box list
            if (vagrant box list | grep -q almalinux); then
              break
            fi
          done;
        '';

        runVagrantAlmalinux = prev.writeScriptBin "run-vagrant-almalinux" ''
          #! ${prev.runtimeShell} -e
          # set -x
          prepare-vagrant-vms \
          && cd "$HOME"/vagrant-examples/libvirt/almalinux/ \
          && vagrant up \
          && vagrant ssh
        '';
            ...
            ({ config, nixpkgs, pkgs, lib, modulesPath, ... }:
              {
                ...
                systemd.user.services.copy-vagrant-examples-vagrant-up = {
                  ...
                  script = ''
                    #! ${pkgs.runtimeShell} -e
                      ...
                      && cd "$BASE_DIR" \
                      && cp -v "${pkgs.vagrantfileAlmalinux}" almalinux/Vagrantfile \
                      && vagrant \
                          box \
                          add \
                ...
                services.xserver.displayManager.sessionCommands = ''
                  ...
                  # Race condition. Why?
                  # sleep 3
                  xdotool type run-vagrant-almalinux \
                  && xdotool key Return
                '';
