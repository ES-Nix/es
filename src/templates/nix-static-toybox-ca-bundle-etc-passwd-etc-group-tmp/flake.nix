{
  description = "OCI image with static Nix, toybox, CA bundle, /etc/passwd, /etc/group and tmp, tested with NixOS test and docker";
  ...
  outputs = { self, nixpkgs, flake-utils }: {
    overlays.default = nixpkgs.lib.composeManyExtensions [
      (final: prev: {
        ...
        testOCIImageNixStaticToybox = prev.testers.runNixOSTest
          {
            name = "test-oci-image-nix-static-toybox";
            nodes.machine =
            ...
            testScript = { nodes, ... }: ''
              ...
              machine.succeed("docker load <${final.OCIImageNixStaticToybox}")
              print(machine.succeed("docker images"))

              image = "nix-static-toybox-ca-bundle-etc-passwd-etc-group-tmp:0.0.1"
              ...
              assert expected in result, f"expected = {expected}, result = {result}"
            '';
          } // { meta.mainProgram = "${final.testOCIImageNixStaticToybox.name}"; };
      ...
      in
      {
        packages = {
          inherit (pkgs)
            caBundleEtcPasswdEtcGroup
            tmpDirs
            OCIImageNixStaticToybox
            testOCIImageNixStaticToybox
            ;
          default = pkgs.testOCIImageNixStaticToybox;
        };
        ...
        apps = {
          default = {
            type = "app";
            program = "${pkgs.lib.getExe pkgs.testOCIImageNixStaticToybox.driverInteractive}";
            meta.description = "Run the testOCIImageNixStaticToybox NixOS test in an interactive mode";
          };
