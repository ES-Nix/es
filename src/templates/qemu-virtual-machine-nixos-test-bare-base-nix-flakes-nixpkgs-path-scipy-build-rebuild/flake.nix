        ...
        testNixOSBare = final.testers.runNixOSTest {
          ...
          nodes = {
            machineABCZ = { config, pkgs, ... }: {
              ...
              system.extraDependencies = with pkgs; [ python3Packages.scipy.inputDerivation ];
          ...
          testScript = { nodes, ... }: ''
            ...
            machineABCZ.succeed("""
              nix build --no-link --print-build-logs --print-out-paths nixpkgs#python3Packages.scipy.dist >&2
            """)
            machineABCZ.succeed("""
              nix build --no-link --print-build-logs --print-out-paths --rebuild nixpkgs#python3Packages.scipy.dist >&2
            """)
