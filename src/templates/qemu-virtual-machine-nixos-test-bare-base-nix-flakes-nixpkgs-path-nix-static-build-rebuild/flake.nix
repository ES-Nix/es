        ...
        testNixOSBare = final.testers.runNixOSTest {
          ...
          nodes = {
            machineABCZ = { config, pkgs, ... }: {
              environment.systemPackages = (with pkgs; [ pkgsStatic.nix ]);

              system.extraDependencies = with pkgs; [ pkgsStatic.nix.inputDerivation ];
          ...
          testScript = { nodes, ... }: ''
            ...
            # machineABCZ.succeed("""
            #   nix build --no-link --print-build-logs --print-out-paths nixpkgs#pkgsStatic.nix >&2
            # """)            
            # machineABCZ.succeed("""
            #   nix build --no-link --print-build-logs --print-out-paths --rebuild nixpkgs#pkgsStatic.nix >&2
            # """)
          '';
        };
