{
  description = "QEMU VM + Docker + pure nixpkgs FastAPI + f2py (Fortran DGEMM) — buildPythonApplication / withPackages / mkDerivation";
  ...
  outputs = { self, nixpkgs, flake-utils }: {
    overlays.default = nixpkgs.lib.composeManyExtensions [
      (final: prev: {
        ...
        matmul-fortran-ext = prev.python3Packages.buildPythonPackage {
          pname = "matmul-fortran";
          version = "0.0.1";
          src = ./app;
          format = "other";
          doCheck = false;
          ...
          configurePhase = ":";
          buildPhase = ''
            python -m numpy.f2py -c matmul.f90 -m matmul_fortran
          '';
          installPhase = ''
            mkdir -p $out/${prev.python3.sitePackages}
            cp matmul_fortran*.so $out/${prev.python3.sitePackages}/
          '';
        };
        ...
        testMyappAsOCIImage = prev.testers.runNixOSTest {
          ...
          testScript = ''
            ...
            if expected_size > 0:
                ...
                print(f"OCI image size (uncalibrated): {actual_size} bytes ({actual_size // 1024 // 1024} MiB) — set expected_size = {actual_size}")
      ...
      in
      {
        ...
        apps = {
          ...
          default = {
            ...
            meta.description = "Run the FastAPI + f2py app (buildPythonApplication)";
          };
          automaticVm = {
            type = "app";
