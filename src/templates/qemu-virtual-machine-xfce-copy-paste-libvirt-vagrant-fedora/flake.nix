  ...
  outputs = { self, nixpkgs, flake-utils }: {
    overlays.default = nixpkgs.lib.composeManyExtensions [
      (final: prev: {
        ...
        # TODO: it is broken
        fedora41 = prev.fetchurl {
          url = "https://vagrantcloud.com/gnome-shell-box/boxes/fedora41/versions/0.0.10/providers/libvirt/amd64/vagrant.box";
          hash = "sha256-xBCTF2HZr3A4hAG+XFJFuL5ygUq8lrC6UF6QzshbAfQ=";
        };

        fedora39 = prev.fetchurl {
          url = "https://vagrantcloud.com/generic/boxes/fedora39/versions/4.3.12/providers/libvirt/amd64/vagrant.box";
          hash = "sha256-VJbWmcy3XiEm7cUAXtod8VlFwsIwnVYlZ/LYTuoj9WI=";
        };

        vagrantfileFedora = prev.writeText "vagrantfile-fedora" ''
        ...
        prepareVagrantVms = prev.writeScriptBin "prepare-vagrant-vms" ''
          ...
          for i in {0..100};do
            ...
            vagrant box list
            if (vagrant box list | grep -q fedora); then
              break
            fi
          done;
        '';

        runVagrantFedora = prev.writeScriptBin "run-vagrant-fedora" ''
          #! ${prev.runtimeShell} -e
          # set -x
          prepare-vagrant-vms \
          && cd "$HOME"/vagrant-examples/libvirt/fedora/ \
          && vagrant up \
          && vagrant ssh
        '';
            ...
            ({ config, nixpkgs, pkgs, lib, modulesPath, ... }:
              {
                ...
                systemd.user.services.copy-vagrant-examples-vagrant-up = {
                  ...
                  script = ''
                    #! ${pkgs.runtimeShell} -e
                      ...
                      && cd "$BASE_DIR" \
                      && cp -v "${pkgs.vagrantfileFedora}" fedora/Vagrantfile \
                      && vagrant \
                          box \
                          add \
                          generic/fedora39 \
                ...
                services.xserver.displayManager.sessionCommands = ''
                  ...
                  # Race condition. Why?
                  # sleep 3
                  xdotool type run-vagrant-fedora \
                  && xdotool key Return
                '';
