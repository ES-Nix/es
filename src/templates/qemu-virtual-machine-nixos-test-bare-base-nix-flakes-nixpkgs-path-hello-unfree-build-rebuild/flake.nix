      ...
      (final: prev: {
        ...
        testNixOSBare = final.testers.runNixOSTest {
          ...
          testScript = { nodes, ... }: ''
            ...
            machineABCZ.succeed("nix --option pure-eval false run nixpkgs#hello-unfree")
            machineABCZ.succeed("nix --option pure-eval false shell nixpkgs#hello-unfree --command hello-unfree")

            machineABCZ.succeed("""
              [ "$(nix-shell -p hello-unfree which --run "which hello")" = "$(nix shell nixpkgs#hello-unfree nixpkgs#which -c which hello-unfree)" ]
            """)

            machineABCZ.succeed("nix --option pure-eval false build --no-link --print-build-logs --print-out-paths nixpkgs#hello-unfree >&2")
            machineABCZ.succeed("nix --option pure-eval false build --no-link --print-build-logs --print-out-paths --rebuild nixpkgs#hello-unfree >&2")
          '';
        };
