{
  description = "A NixOS VM with XFCE, copy/paste support, Libvirt and Vagrant with Archlinux";
    ...
    overlays.default = nixpkgs.lib.composeManyExtensions [
      (final: prev: {
        ...
        archlinux = prev.fetchurl {
          ...
          url = "https://vagrantcloud.com/boxen/boxes/archlinux-rolling/versions/2025.04.02.21/providers/libvirt/amd64/vagrant.box";
          hash = "sha256-jz+fId1oc7cCjh+aU0ETkQir+8zLZ9KkdYmCAhw8hjQ=";
        };

        # vagrantfiles
        vagrantfileArchlinux = prev.writeText "vagrantfile-archlinux" ''
        ...
        prepareVagrantVms = prev.writeScriptBin "prepare-vagrant-vms" ''
          ...
          for i in {0..100};do
            ...
            vagrant box list
            if (vagrant box list | grep -q archlinux); then
              break
            fi
          done;
        '';

        runVagrantArchlinux = prev.writeScriptBin "run-vagrant-archlinux" ''
          #! ${prev.runtimeShell} -e
          # set -x
          prepare-vagrant-vms \
          && cd "$HOME"/vagrant-examples/libvirt/archlinux/ \
          && vagrant up \
          && vagrant ssh
        '';
            ...
            ({ config, nixpkgs, pkgs, lib, modulesPath, ... }:
              {
                ...
                systemd.user.services.copy-vagrant-examples-vagrant-up = {
                  ...
                  script = ''
                    #! ${pkgs.runtimeShell} -e
                      ...
                      cp -v "${pkgs.vagrantfileArchlinux}" archlinux/Vagrantfile
                ...
                services.xserver.displayManager.sessionCommands = ''
                  ...
                  # Race condition. Why?
                  # sleep 3
                  xdotool type run-vagrant-archlinux \
                  && xdotool key Return
                '';
