...
let
  ...
  off-install-script = pkgs.writeScript "off-install-script" ''
    ...
    parted -s /dev/sda -- mkpart primary 512MiB -1GiB
    parted -s /dev/sda -- mkpart primary linux-swap -1GiB -500MiB
    ...
    # TODO: Labels do not appear immediately, so wait a moment
    sleep 5
    ...
    echo "${pkgs.path}"
    nix build --offline --no-use-registries --file "${pkgs.path}"'/nixos' system \
    -I nixos-config=/mnt/etc/nixos/configuration.nix -o /out
