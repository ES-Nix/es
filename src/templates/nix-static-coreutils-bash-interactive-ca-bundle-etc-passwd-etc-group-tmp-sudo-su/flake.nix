{
  description = "OCI image with static Nix, coreutils, bashInteractive, su, sudo, CA bundle, /etc/passwd, /etc/group and tmp, tested with NixOS test and docker";
  ...
  outputs = { self, nixpkgs, flake-utils }: {
    overlays.default = nixpkgs.lib.composeManyExtensions [
      (final: prev: {
        ...
        OCIImageNixStaticCoreutilsBashInteractiveSudoSu = prev.dockerTools.buildImage {
          name = "nix-static-coreutils-bash-interactive-ca-bundle-etc-passwd-etc-group-tmp-sudo-su";
          tag = "0.0.1";
          copyToRoot = [
        ...
        testOCIImageNixStaticCoreutilsBashInteractiveSudoSu = prev.testers.runNixOSTest
          {
            name = "test-oci-image-nix-static-coreutils-bash-interactive-sudo-su";
            nodes.machine =
            ...
            testScript = { nodes, ... }: ''
              ...
              machine.succeed("docker load <${final.OCIImageNixStaticCoreutilsBashInteractiveSudoSu}")
              print(machine.succeed("docker images"))

              image = "nix-static-coreutils-bash-interactive-ca-bundle-etc-passwd-etc-group-tmp-sudo-su:0.0.1"

              result = machine.succeed(f"docker run --rm {image} bash -c 'nix --version'")
              expected = 'nix (Nix) ${prev.nixStatic.version}'
              assert expected in result, f"expected = {expected}, result = {result}"

              result = machine.succeed(f"docker run --rm {image} bash -c 'ls --version'")
              expected = 'GNU coreutils'
              assert expected in result, f"expected = {expected}, result = {result}"

              result = machine.succeed(f"docker run --rm {image} bash -c 'sudo --version'")
              expected = 'Sudo version'
              assert expected in result, f"expected = {expected}, result = {result}"

              result = machine.succeed(f"docker run --rm {image} bash -c 'whoami'")
              expected = 'root'
              assert expected in result, f"expected = {expected}, result = {result}"
              ...
              result = machine.succeed(f"docker run --rm {image} bash -c 'echo $USER'")
              expected = 'nixuser'
              assert expected in result, f"expected = {expected}, result = {result}"
            '';
          } // { meta.mainProgram = "${final.testOCIImageNixStaticCoreutilsBashInteractiveSudoSu.name}"; };
      ...
      in
      {
        packages = {
          inherit (pkgs)
            caBundleEtcPasswdEtcGroup
            tmpDirs
            OCIImageNixStaticCoreutilsBashInteractiveSudoSu
            testOCIImageNixStaticCoreutilsBashInteractiveSudoSu
            ;
          default = pkgs.testOCIImageNixStaticCoreutilsBashInteractiveSudoSu;
        };
        ...
        apps = {
          default = {
            type = "app";
            program = "${pkgs.lib.getExe pkgs.testOCIImageNixStaticCoreutilsBashInteractiveSudoSu.driverInteractive}";
            meta.description = "Run the testOCIImageNixStaticCoreutilsBashInteractiveSudoSu NixOS test in an interactive mode";
          };
