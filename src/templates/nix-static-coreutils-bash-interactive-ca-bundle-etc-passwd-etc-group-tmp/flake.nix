{
  description = "OCI image with static Nix, coreutils, bashInteractive, CA bundle, /etc/passwd, /etc/group and tmp, tested with NixOS test and docker";
  ...
  outputs = { self, nixpkgs, flake-utils }: {
    overlays.default = nixpkgs.lib.composeManyExtensions [
      (final: prev: {
        ...
        OCIImageNixStaticCoreutilsBashInteractive = prev.dockerTools.buildImage {
        ...
        testOCIImageNixStaticCoreutilsBashInteractive = prev.testers.runNixOSTest
          {
            name = "test-oci-image-nix-static-coreutils-bash-interactive";
            nodes.machine =
            ...
            testScript = { nodes, ... }: ''
              ...
              machine.succeed("docker load <${final.OCIImageNixStaticCoreutilsBashInteractive}")
              print(machine.succeed("docker images"))

              image = "nix-static-coreutils-bash-interactive-ca-bundle-etc-passwd-etc-group-tmp:0.0.1"
              ...
              assert expected in result, f"expected = {expected}, result = {result}"
            '';
          } // { meta.mainProgram = "${final.testOCIImageNixStaticCoreutilsBashInteractive.name}"; };
      ...
      in
      {
        packages = {
          inherit (pkgs)
            caBundleEtcPasswdEtcGroup
            tmpDirs
            OCIImageNixStaticCoreutilsBashInteractive
            testOCIImageNixStaticCoreutilsBashInteractive
            ;
          default = pkgs.testOCIImageNixStaticCoreutilsBashInteractive;
        };
        ...
        apps = {
          default = {
            type = "app";
            program = "${pkgs.lib.getExe pkgs.testOCIImageNixStaticCoreutilsBashInteractive.driverInteractive}";
            meta.description = "Run the testOCIImageNixStaticCoreutilsBashInteractive NixOS test in an interactive mode";
          };
