      ...
      (final: prev: {
        ...
        testNixOSBare = final.testers.runNixOSTest {
          ...
          testScript = { nodes, ... }: ''
            ...
            machineABCZ.succeed("""
              nix build --no-link --print-build-logs --print-out-paths nixpkgs#python3Packages.scipy >&2
            """)
            machineABCZ.succeed("""
              nix build --no-link --print-build-logs --print-out-paths --rebuild nixpkgs#python3Packages.scipy >&2
            """)
          '';
        };
