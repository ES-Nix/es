{
  description = "OCI image with static Nix, busybox (full), su, sudo, CA bundle, /etc/passwd, /etc/group and tmp, tested with NixOS test and docker";
  ...
  outputs = { self, nixpkgs, flake-utils }: {
    overlays.default = nixpkgs.lib.composeManyExtensions [
      (final: prev: {
        ...
        OCIImageNixStaticBusyboxSudoSu = prev.dockerTools.buildImage {
          name = "nix-static-busybox-ca-bundle-etc-passwd-etc-group-tmp-sudo-su";
          tag = "0.0.1";
          copyToRoot = [
        ...
        testOCIImageNixStaticBusyboxSudoSu = prev.testers.runNixOSTest
          {
            name = "test-oci-image-nix-static-busybox-sudo-su";
            nodes.machine =
            ...
            testScript = { nodes, ... }: ''
              ...
              machine.succeed("docker load <${final.OCIImageNixStaticBusyboxSudoSu}")
              print(machine.succeed("docker images"))

              image = "nix-static-busybox-ca-bundle-etc-passwd-etc-group-tmp-sudo-su:0.0.1"
              ...
              assert expected in result, f"expected = {expected}, result = {result}"
            '';
          } // { meta.mainProgram = "${final.testOCIImageNixStaticBusyboxSudoSu.name}"; };
      ...
      in
      {
        packages = {
          inherit (pkgs)
            caBundleEtcPasswdEtcGroup
            tmpDirs
            OCIImageNixStaticBusyboxSudoSu
            testOCIImageNixStaticBusyboxSudoSu
            ;
          default = pkgs.testOCIImageNixStaticBusyboxSudoSu;
        };
        ...
        apps = {
          default = {
            type = "app";
            program = "${pkgs.lib.getExe pkgs.testOCIImageNixStaticBusyboxSudoSu.driverInteractive}";
            meta.description = "Run the testOCIImageNixStaticBusyboxSudoSu NixOS test in an interactive mode";
          };
