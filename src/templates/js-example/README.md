...
Use the bunx version, as it is not interactive.
```bash
# bun create vue@latest
# bun create vue@latest -- --template vue 
...
```bash
bunx create-vite my-vue-app --template vue-ts \
&& cd my-vue-app \
&& npm install \
&& npm run dev
```
    ...
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        frontend = pkgs.stdenv.mkDerivation {
          name = "frontend";
          src = ./.;
          packages = [ pkgs.yarn nodeModules ];
          buildPhase = ''
...
docker run ---interactive=true --tty=true --rm=true python312 -c 'import this'
```
