{
  description = "OCI image with static Nix, busybox-sandbox-shell, su, sudo, CA bundle, /etc/passwd, /etc/group and tmp, tested with NixOS test and docker";
  ...
  outputs = { self, nixpkgs, flake-utils }: {
    overlays.default = nixpkgs.lib.composeManyExtensions [
      (final: prev: {
        ...
        OCIImageNixStaticBusyboxSandboxShellSudoSu = prev.dockerTools.buildImage {
        ...
        testOCIImageNixStaticBusyboxSandboxShellSudoSu = prev.testers.runNixOSTest
          {
            name = "test-oci-image-nix-static-busybox-sandbox-shell-sudo-su";
            nodes.machine =
            ...
            testScript = { nodes, ... }: ''
              ...
              machine.succeed("docker load <${final.OCIImageNixStaticBusyboxSandboxShellSudoSu}")
              print(machine.succeed("docker images"))

              image = "nix-static-busybox-sandbox-shell-ca-bundle-etc-passwd-etc-group-tmp-sudo-su:0.0.1"
              ...
              assert expected in result, f"expected = {expected}, result = {result}"
            '';
          } // { meta.mainProgram = "${final.testOCIImageNixStaticBusyboxSandboxShellSudoSu.name}"; };
      ...
      in
      {
        packages = {
          inherit (pkgs)
            caBundleEtcPasswdEtcGroup
            tmpDirs
            OCIImageNixStaticBusyboxSandboxShellSudoSu
            testOCIImageNixStaticBusyboxSandboxShellSudoSu
            ;
          default = pkgs.testOCIImageNixStaticBusyboxSandboxShellSudoSu;
        };
        ...
        apps = {
          default = {
            type = "app";
            program = "${pkgs.lib.getExe pkgs.testOCIImageNixStaticBusyboxSandboxShellSudoSu.driverInteractive}";
            meta.description = "Run the testOCIImageNixStaticBusyboxSandboxShellSudoSu NixOS test in an interactive mode";
          };
