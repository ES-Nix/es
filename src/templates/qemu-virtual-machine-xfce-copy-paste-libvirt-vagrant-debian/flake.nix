    ...
    overlays.default = nixpkgs.lib.composeManyExtensions [
      (final: prev: {
        ...
        debian = prev.fetchurl {
          url = "https://vagrantcloud.com/alvistack/boxes/debian-12/versions/20260106.1.1/providers/libvirt/amd64/vagrant.box";
          ...
          # url = "https://vagrantcloud.com/alvistack/boxes/debian-12/versions/20250517.1.1/providers/libvirt/amd64/vagrant.box";
          hash = "sha256-OfwToDtINf0lTdjmXSILQhyzd9yGhOzvWRaSs73XoYI=";
        };
        ...
        prepareVagrantVms = prev.writeScriptBin "prepare-vagrant-vms" ''
          ...
          for i in {0..100};do
            ...
            vagrant box list
            if (vagrant box list | grep -q debian); then
              break
            fi
          done;
        '';

        runVagrantDebian = prev.writeScriptBin "run-vagrant-debian" ''
          #! ${prev.runtimeShell} -e
          # set -x
          prepare-vagrant-vms \
            ...
            ({ config, nixpkgs, pkgs, lib, modulesPath, ... }:
              {
                ...
                systemd.user.services.copy-vagrant-examples-vagrant-up = {
                  ...
                  script = ''
                    #! ${pkgs.runtimeShell} -e
                      ...
                      && cd "$BASE_DIR" \
                      && cp -v "${pkgs.vagrantfileDebian}" debian/Vagrantfile \
                      && chmod 0644 debian/Vagrantfile \
                      && vagrant \
                          box \
                          add \
                          alvistack/debian-12 \
                ...
                services.xserver.displayManager.sessionCommands = ''
                  ...
                  # Race condition. Why?
                  # sleep 3
                  xdotool type run-vagrant-debian \
                  && xdotool key Return
                '';
