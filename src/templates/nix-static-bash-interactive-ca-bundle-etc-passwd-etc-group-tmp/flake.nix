{
  description = "OCI image with static Nix, bashInteractive, CA bundle, /etc/passwd, /etc/group and tmp, tested with NixOS test and docker";
  ...
  outputs = { self, nixpkgs, flake-utils }: {
    overlays.default = nixpkgs.lib.composeManyExtensions [
      (final: prev: {
        ...
        OCIImageNixStaticBashInteractive = prev.dockerTools.buildImage {
        ...
        testOCIImageNixStaticBashInteractive = prev.testers.runNixOSTest
          {
            name = "test-oci-image-nix-static-bash-interactive";
            nodes.machine =
            ...
            testScript = { nodes, ... }: ''
              ...
              machine.succeed("docker load <${final.OCIImageNixStaticBashInteractive}")
              print(machine.succeed("docker images"))

              image = "nix-static-bash-interactive-ca-bundle-etc-passwd-etc-group-tmp:0.0.1"
              ...
              assert expected in result, f"expected = {expected}, result = {result}"
            '';
          } // { meta.mainProgram = "${final.testOCIImageNixStaticBashInteractive.name}"; };
      ...
      in
      {
        packages = {
          inherit (pkgs)
            caBundleEtcPasswdEtcGroup
            tmpDirs
            OCIImageNixStaticBashInteractive
            testOCIImageNixStaticBashInteractive
            ;
          default = pkgs.testOCIImageNixStaticBashInteractive;
        };
        ...
        apps = {
          default = {
            type = "app";
            program = "${pkgs.lib.getExe pkgs.testOCIImageNixStaticBashInteractive.driverInteractive}";
            meta.description = "Run the testOCIImageNixStaticBashInteractive NixOS test in an interactive mode";
          };
