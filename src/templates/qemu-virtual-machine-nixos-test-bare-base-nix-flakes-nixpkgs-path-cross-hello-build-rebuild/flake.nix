        ...
        testNixOSBare = final.testers.runNixOSTest {
          ...
          nodes = {
            machineABCZ = { config, pkgs, ... }: {
              environment.systemPackages = (with pkgs; [ pkgsCross.aarch64-multiplatform.hello nix ]);

              system.extraDependencies = with pkgs; [
                pkgsCross.aarch64-multiplatform.hello.inputDerivation
              ];
          ...
          testScript = { nodes, ... }: ''
            machineABCZ.succeed("""
              nix build --no-link --print-build-logs --print-out-paths nixpkgs#pkgsCross.aarch64-multiplatform.hello >&2
            """)
            machineABCZ.succeed("""
              nix build --no-link --print-build-logs --print-out-paths --rebuild nixpkgs#pkgsCross.aarch64-multiplatform.hello >&2
            """)
          '';
        };
      ...
      in
      {
        ...
        apps = {
          ...
          default = {
            ...
            meta.description = "Run the interactive driver for the bare base test";
          };
        };
